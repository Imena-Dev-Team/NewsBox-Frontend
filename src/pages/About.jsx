...
const About = () => {
      ...
      children: [
        {
          ...
          children: [
            level3Nodes[index * 4],
            level3Nodes[index * 4 + 1]
          ]
        },
        {
          ...level2Nodes[index * 2 + 1],
          children: [
            level3Nodes[index * 4 + 2],
            level3Nodes[index * 4 + 3]
          ]
        }
      ]
    }));
  ...
  const BinaryTreeNode = ({ node, level = 0, position = 'center' }) => {
    const getNodeColor = () => {
      if (level === 0) return 'bg-gradient-to-br from-blue-600 to-purple-600';
      if (level === 1) return 'bg-gradient-to-br from-green-500 to-blue-500';
      return 'bg-gradient-to-br from-purple-500 to-pink-500';
    };

    const getNodeSize = () => {
      if (level === 0) return 'w-16 h-16 text-lg';
      if (level === 1) return 'w-14 h-14 text-base';
      return 'w-12 h-12 text-sm';
    };

    return (
      <div className={`flex flex-col items-center ${position === 'left' ? 'items-start' : position === 'right' ? 'items-end' : 'items-center'}`}>
        {/* Node Circle */}
        <div className={`${getNodeSize()} ${getNodeColor()} rounded-full flex items-center justify-center text-white font-bold shadow-lg mb-2 relative`}>
          {node.name?.charAt(0)?.toUpperCase() || 'M'}
        </div>
        
        {/* Node Info */}
        <div className="text-center mb-4 max-w-32">
          <h3 className="font-semibold text-gray-800 text-xs sm:text-sm mb-1">
            {node.name}
          </h3>
          {node.role && (
            <p className="text-xs text-gray-600 mb-1">{node.role}</p>
          )}
          {node.birthday && (
            <p className="text-xs text-gray-500">
              {new Date(node.birthday).getFullYear()}
            </p>
          )}
        </div>
          ...
          <div className="flex justify-center space-x-8 mt-4">
            {node.children.map((child, index) => (
              <div key={child.id} className="relative">
                {/* Connection Line */}
                <div className="absolute -top-4 left-1/2 w-px h-4 bg-gray-400" style={{ transform: 'translateX(-50%)' }}></div>
                {index === 0 && (
                  <div className="absolute -top-4 left-1/2 w-8 h-px bg-gray-400" style={{ transform: 'translateX(-50%) translateX(-2rem)' }}></div>
                )}
                {index === 1 && (
                  <div className="absolute -top-4 left-1/2 w-8 h-px bg-gray-400" style={{ transform: 'translateX(-50%) translateX(2rem)' }}></div>
                )}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-800 mb-2">Error Loading Family Tree</h2>
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-8">
            Discover the connections and relationships within our extended family. 
            Each level represents a generation, and each circle represents a family member.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
          <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 text-center border border-white/30 shadow-lg">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg mx-auto mb-3">
              {selectedYear}
            </div>
            <h3 className="font-semibold text-gray-800 mb-1">Selected Year</h3>
            <p className="text-sm text-gray-600">Current family view</p>
          </div>
          
          <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 text-center border border-white/30 shadow-lg">
            <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg mx-auto mb-3">
              14
            </div>
            <h3 className="font-semibold text-gray-800 mb-1">Total Members</h3>
            <p className="text-sm text-gray-600">Family members in this tree</p>
          </div>
          
          <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 text-center border border-white/30 shadow-lg">
            <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg mx-auto mb-3">
              3
            </div>
            <h3 className="font-semibold text-gray-800 mb-1">Generations</h3>
            <p className="text-sm text-gray-600">Levels in the family tree</p>
          </div>
        </div>
        ...
        {/* Legend */}
        <div className="mt-12 bg-white/70 backdrop-blur-sm rounded-xl p-6 border border-white/30 shadow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-700">Elders/Adults (Level 1)</span>
            </div>
            <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm text-gray-700">Grandchildren (Level 3)</span>
            </div>
          </div>
          <div className="mt-4 text-center">
            <p className="text-sm text-gray-600">
              Each year shows different family members in the same binary tree structure
            </p>
          </div>
        </div>
      </div>
