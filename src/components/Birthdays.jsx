        ...
        {loading ? (
          ...
          <div className="font-['Poppins',sans-serif] p-10 ml-[14px] box-border mt-8">
            ...
            <div className="font-['Poppins',sans-serif] p-10 ml-[14px] box-border mt-24">
              ...
              <div className="relative">
                <img src={Image} className="absolute right-0 -top-[170px] text-[70px] -z-10 opacity-90" />
                <div className="flex flex-wrap gap-[30px] justify-between w-full">
                  {messages.length === 0 ? (
                    <div className="flex-[30%] p-5 rounded-[15px] box-border shadow-[0px_4px_10px_rgba(0,0,0,0.1)]">
                    </div>
                  ) : (
                    messages.map((message) => (
                      <div key={message.id} className="bg-white flex-[30%] p-5 rounded-[15px] box-border shadow-[0px_4px_10px_rgba(0,0,0,0.1)]">
                        ...
                        <p className="text-xs text-[#888] mt-2.5 leading-normal"> Posted on: {message.timestamp} {message.author && ( <span className="ml-2 text-[#2aa2ff] font-semibold"> — {message.author} </span> )} </p>
                      </div>
                    ))
                  )}
                </div>
                <img src={MyImage} alt="Gift" className="absolute top-[250px] -left-[112px] w-[120px] -z-10 opacity-90 -translate-y-[20%] ml-[15px] mb-[90px]" />
                <form onSubmit={handleSubmit} className="mt-6 relative font-['Poppins',sans-serif] flex flex-col items-center gap-2">
                  <input type="text" placeholder="Your name" value={author} onChange={(e) => setAuthor(e.target.value)} className="w-[180px] h-[36px] text-base font-normal border border-[#d3d3d3] rounded px-3 py-1 mb-1 focus:border-[#2aa2ff] outline-none mt-6" maxLength={20} required />
                  <input type="text" placeholder="Click to type birthday wishes to the born baby" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} className="w-[70%] h-[50px] text-2xl font-medium font-['Poppins',sans-serif] border-none border-l-3 border-[#d3d3d3] outline-none px-5 py-2.5 mx-auto my-[30px] block text-[#333] bg-white mt-4" required />
                  <button type="submit" className={`absolute right-5 top-[100px] w-[200px] h-[50px] rounded-[40px] cursor-pointer text-xl transition-all duration-300 ease-in-out border border-[#2aa2ff] ${clicked ? "bg-[#2aa2ff] text-white" : "bg-white text-[#2aa2ff]"}`}> POST </button>
                </form>
                <div className="mb-12" />
              </div>
            </div>
